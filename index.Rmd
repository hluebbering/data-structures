...
The goal of the following project is to use Python to analyze and interact with data in a real-world scenario. Specifically, we write Python code using loops, conditionals (if statements), functions, and string manipulation. We then use the command line to run Python scripts with parameters.



<span class = "myhighlight3">Overview:</span> Using Python, we implement a program to process and analyze data for Pokémon Go, an augmented-reality video game played by hundreds of millions of people. The problems in this assessment focus on providing descriptive statistics for summarizing the Pokémon dataset.
...
The program reads, processes, and groups CSV data storing information about pokemon to compute descriptive statistics. 
...
We organize the functions for solving each problem into multiple python files described below. 
...
def filename_to_string(dna_filename):
    ...
    for line in inputfile:
        line_num = line_num + 1
        if line_num % 4 == 2: # 2nd, 6th, 10th line...
            # Remove characters from end of line
            line = line.rstrip()
            # Concatenate line to end of current string
            seq = seq + line
...
Next, we write at least one test function for each problem, giving it a descriptive name such as *test_species_count*.
...
<span class = "myhighlight4">Task:</span> 
For this assignment, we provide the program with the name of the data file so it knows where to read data from. We do this in the main program by reading in a data file name as an argument from the command line.
...
# Save the 1st provided argument as a string
file_name = sys.argv[1]

# Open file and read all nucleotides into a string of letters
nucleotides = filename_to_string(file_name)
```


Now, let's compare the output of our code to output produced from a test file small enough so that we can easily calculate the GC content by hand. Using the sample data file as input to our program, we run the following command in the terminal to verify that it provides the correct answer.
